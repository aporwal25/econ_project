...
def analyze_tfrecord_batch(iter_init: tf.Operation,
    ...
    mins = np.ones(nbands, dtype=np.float32) * np.inf
    mins_nz = np.ones(nbands, dtype=np.float32) * np.inf
    mins_goodpx = np.ones(nbands, dtype=np.float32) * np.inf
    maxs = np.zeros(nbands, dtype=np.float32)
    sums = np.zeros(nbands, dtype=np.float32)
    sum_sqs = np.zeros(nbands, dtype=np.float32)
    nz_pixels = np.zeros(nbands, dtype=np.int32)
        ...
        while True:
            try:
                ...
                for mask, bands in [(dmsp_mask, dmsp_bands), (viirs_mask, viirs_bands)]:
                    ...
                    sums[bands] += np.sum(imgs_nonneg, axis=(0,1,2), dtype=np.float32)
                    sum_sqs[bands] += np.sum(imgs_nonneg ** 2, axis=(0,1,2), dtype=np.float32)
